...
import IdentityCard from './validate/IdentityCard.js';
...
class Person {
  ...
  validate(field) {
    if (!field || field === '*') {
      const all = ['name', 'credential', 'gender', 'birthday', 'mobile', 'kinship'];
      ...
      switch (field) {
        ...
        case 'kinship':
          return new ValidationResult(!!this.kinship, '请选择关系');
        default:
