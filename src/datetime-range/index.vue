...
import { DatetimeSelect } from '@njzhyl/common-component-vant';
...
export default {
  ...
  computed: {
    ...
    maxDate() {
      const endTime = this.datetimeRange.endDate || this.rangeMaxDate;
  ...
  created() {
    logger.debug('DatetimeRange.created');
    this.getDateShow();
  },
  methods: {
    getDateShow() {
      this.datetimeRange = clone(this.value);
    },
    // 改变开始时间
    changeStart() {
      logger.debug('DatetimeRange.changeStart startDate = {0}', this.datetimeRange.startDate);
      this.change();
      this.$emit('start', this.datetimeRange.startDate);
    },
    // 改变结束时间
    changeEnd() {
      logger.debug('DatetimeRange.changeEnd endDate = {0}', this.datetimeRange.endDate);
      this.change();
      this.$emit('end', this.datetimeRange.endDate);
    },
    // 更新时间
    change() {
